...
export class Cart extends Component {
  ...
  constructor(parentNode: HTMLElement) {
    ...
    this.totalItems =
      this.cartList.reduce((acc: number, el: Wine) => acc + el.inCart, 0) || 0;
    this.data = LocalStorage.getLocalStorage(localStorageKeys.goods) || goods;
    this.totalLabel = new Component(
      this.node,
      "span",
      "cart__products",
      `${this.totalItems}`
    ...
    this.cartPopupContainer = new Component(
      this.cartPopup.node,
      "div",
  ...
  render() {
    ...
    const closePopup = new Component(
      this.cartPopup.node,
      "span",
      "close-popup",
      "X"
    ...
    this.cartList.forEach((item) => {
      const cartItem = new Component(
        this.cartPopupContainer.node,
        "div",
        "cart-item"
      );
      cartItem.node.setAttribute("id", `${item.id}`);
      const cartItemName = new Component(
        cartItem.node,
        "p",
        "cart-item__name",
        `${item.name}`
      ...
      const removeBtn = new Component(
        cartNumber.node,
        "button",
        "cart__remove cart__btn",
        "-"
      );
      removeBtn.node.setAttribute("data-articul", `${item.id}`);
      const cartInput = new Component(
        cartNumber.node,
        "p",
        "cart__input",
        `${item.inCart.toString()}`
      );
      ...
      const cartItemPrice = new Component(
        cartItem.node,
        "p",
        "cart-item__price",
        `${itemsPrice}`
